...
class Advise(object):
    ...
    expire_binlog_days_warning = "您的配置参数expire_logs_days设置为{0}，" \
                                 "建议设置为{1}，否则存在binlog占用太多磁盘空间的风险"
    ...
    innodb_flush_method = "您的数据库配置参数innodb_flush_method取值为{0}，建议您修改该值为{1}，" \
                          "否则当操作系统宕机时，您的数据存在丢失风险"

    innodb_log_file_size = "您的磁盘空间为{0}，您的数据库配置参数innodb_log_file_size取值为{1}，" \
                           "该值太大会影响数据库故障恢复的速度，该值太小会影响数据库的性能，" \
                           "根据您的磁盘空间，我们建议您取值为{2}到{3}之间"

    max_connection_warning = "数据库连接数过多容易造成线程频繁的上下文切换，" \
                             "连接数过少不能充分发挥数据库的性能，您的数据库连接数是{0}，" \
    ...
    relay_log_recovery = "您的从机配置参数relay_log_recovery设置为{0}，" \
                         "从机宕机后存在主从数据不一致的风险，请修改该参数为{1}"

    relay_log_info_repository = "您的从机配置参数relay_log_info_repository设置为{0}，" \
                                "从机宕机后存在主从数据不一致的风险，请修改该参数为{1}"
